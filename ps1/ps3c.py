...
def house_hunting(annual_salary, total_cost=1e6, semi_annual_raise=0.07):
    ...
    while not current_savings - 100 < down_payment < current_savings + 100:
        month = 0
        current_savings = 0.
        start_salary = annual_salary
        middle_portion_saved = (upper_portion_saved + lower_portion_saved) / 2
        count += 1
        while month != target_month:
            month += 1
            if month % 6 == 1 and month > 6:
                start_salary += start_salary * semi_annual_raise
            current_savings = current_savings + (start_salary / 12) * middle_portion_saved + current_savings * r
    ...
    return middle_portion_saved, count
...
Steps in bisecion search: {1:}'''.format(house_hunting(annual_salary)[0], house_hunting(annual_salary)[1]))